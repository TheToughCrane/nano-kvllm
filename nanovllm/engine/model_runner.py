...
class ModelRunner:
    ...
    def allocate_kv_cache(self):
        ...
        if config.kv_compress_enabled:#create q_cache if we compress KV cache
            # self.q_cache = torch.empty(hf_config.num_hidden_layers,config.num_query_blocks, self.block_size, num_kv_heads, head_dim)
            self.q_cache = torch.empty(hf_config.num_hidden_layers, config.num_query_blocks,
        ...
        for module in self.model.modules():
            if hasattr(module, "k_cache") and hasattr(module, "v_cache"):
                ...
                if config.kv_compress_enabled:#newly added code
